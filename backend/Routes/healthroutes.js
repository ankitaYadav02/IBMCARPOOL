...
router.post('/healthdetails',requireLogin,(req,res)=>{
    const {DryCough,tiredness,fever,sorethroat,diarrhoea,postedBy} = req.body
...
router.get('/health',requireLogin,(req,res)=>{
    ...
    health.findOne({postedBy:req.user._id})
    .then(Health=>{
        res.json(Health);
    })
    .catch(err=>{
...
router.put('/updatehealth',requireLogin,(req,res)=>{
    console.log(req.user._id)
    health.findOneAndUpdate({postedBy:req.user._id},{$set:{
